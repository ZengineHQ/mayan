...
const { sleep, printErrorAndAbort, safeJSONParse } = require('../util')
  ...
  const deployFrontend = frontend => {
    ...
    const recursivelyCheckDeployment = async () => { // eslint-disable-line
      ...
      if (unparsed instanceof Error) {
        return Promise.reject(unparsed)
      }

      const res = safeJSONParse(unparsed)

      if (res.status < 200 || 299 < res.status) { // eslint-disable-line
        return Promise.reject(new Error('unable to successfully fetch plugin deploy status:' + JSON.stringify(res)))
      }
      ...
      if (res.data && (res.data.deployStatus === 'in-progress' || res.data.deployStatus === 'pending')) {
        console.log(yellow(`Deployment is ${res.data.deployStatus.split('-').join(' ')}`))
      ...
      console.error(red('Something went wrong fetching the deploy status.'))
      console.info('Check your configuration details and try again.')

      Promise.reject(new Error('Something went wrong fetching the deploy status.'))
    }
    ...
    const onError = (err) => {
      ...
      switch (errorCode) {
        ...
        case 4013:
          message = red(format(
            'Unauthorized request, verify that you have permission to manage the plugin.'
          ))
          break
        ...
        default:
          message = format(
            '%s\n\n %s',
            red('Unknown error occurred, check the http response below'),
            errorResponse
          )
      }

      console.log(message)
      printErrorAndAbort('deploying frontend')(errorResponse)
    }
