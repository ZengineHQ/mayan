 ...
 * @returns {Promise<any>}
 */
const buildModules = (src, dest, destPkg) => {
  ...
  return new Promise((resolve, reject) => {
    return destCheckSum !== srcCheckSum && destExists ? rimraf(dest, err => err ? reject(err) : resolve()) : resolve()
  }).then(() => {
    // if checksums match, skip npm installation and any package.json copying
    if (destCheckSum === srcCheckSum) {
      return Promise.resolve()
    }

    // Recreate dest directory, if necessary.
    if (!destExists) {
      mkdirp.sync(dest)
    }
