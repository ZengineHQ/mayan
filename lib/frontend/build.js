...
const { runScript, printErrorAndAbort } = require('../util')

const onBuildError = printErrorAndAbort('building frontend')
    ...
    if (frontend.version === 2) {
      return new Promise((resolve, reject) => {
        ...
        spawnedProcess.on('close', code => {
          return code > 0 ? reject(new Error(`exit code: ${code}`)) : resolve(code)
        })
      })
        .then(() => {
