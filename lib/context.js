...
function createContext (ctx) {
  ...
  if (!ctx.config) {
    ...
    const config = fs.existsSync(ctx.config)
  ...
  const environmentName = ctx.env ? ctx.env : getDefaultEnvName(ctx)
  ...
  const environment = _.get(ctx.environments, environmentName)
  ...
  if (ctx.plugin !== '*') {
    ctx.plugins = { [ctx.plugin]: _.merge({ configName: ctx.plugin }, _.get(ctx.plugins, ctx.plugin)) }
  }
