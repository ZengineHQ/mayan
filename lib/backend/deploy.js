...
const { printErrorAndAbort, safeJSONParse } = require('../util')
    ...
    const deployBackend = (backend) => {
      ...
      let onError = (err) => {
        try {
          ...
          console.log(message)
          printErrorAndAbort('deploying backend')(errJson)

        } catch (e) {
          message = kleur.red(format('An un parseable error occurred: %s', err))
          console.log(message)
          printErrorAndAbort('deploying backend')(message)
        }
