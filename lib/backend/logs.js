...
module.exports = async argv => {
  ...
  if (!service) {
    console.log(red(format('\r\nNo matching service %s found check your maya.json', argv.service)))
    return;
  }

  console.log(green(format('Getting Logs for service %s/%d', service.configName, service.id)))
  ...
  }).catch(e => console.log(red(`Error: ${e ? e.message : ' unknown'}`)))

  if (!uuid) {
    console.log(red(format('Could not retrieve Log Group Name check your service name and id matches in your maya.json', argv.service)))
    return;
  }
  const tailArgs = [
    'logs',
