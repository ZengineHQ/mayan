...
module.exports = async argv => {
  ...
  if (!uuid) {
    console.log(kleur.red(format('Cound not retrieve Log Group Name check your service name and id matches in your maya.json', argv.service)))
    return;
  }
  const tailArgs = [
    'logs',
    'tail',
    `/aws/lambda/${envPrefix}_plugin_${uuid.replaceAll('-', '_')}`,
    '--follow',
    `--format=${argv.format || 'short'}`,
    `--since=${argv.since || 5}h`
  ]
