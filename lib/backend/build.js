    ...
    return new Listr([
      {
        ...
        task: (ctx, task) => runScript(src, 'maya-pre-build', ctx.env).then(res => handleScriptOutput(res, task))
      },
      {
        title: 'Copying files',
      ...
      },
      {
        ...
        task: (ctx, task) => runScript(src, ['maya-post-build', 'maya-build'], ctx.env).then(res => handleScriptOutput(res, task))
      }
