...
const { printErrorAndAbort, runScript, setNgrokUrl, checkNvm, checkNpmVersion } = require('../util')
...
module.exports = async argv => {
  ...
  if (ctx.proxy_settings && ctx.proxy_settings.authtoken && (ctx.proxy_settings.subdomain || ctx.proxy_settings.domain)) {
    ngrokOptions.authtoken = ctx.proxy_settings.authtoken
    if (ctx.proxy_settings.subdomain) {
      ngrokOptions.domain = `${ctx.proxy_settings.subdomain}.ngrok.io`;
    } else {
      ngrokOptions.domain = ctx.proxy_settings.domain
    }
  }

  const listener = await ngrok.forward(ngrokOptions).catch(err => err instanceof Error ? err : new Error(err))

  if (listener instanceof Error) throw listener
