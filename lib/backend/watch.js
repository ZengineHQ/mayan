...
const onProxyError = printError('backend http proxy')
...
module.exports = async argv => {
  ...
  proxy.on('error', (err, req, res) => onProxyError(err))
  ...
  function tearDownListeners (proxyServers) {
    ...
    process.on('uncaughtException', error => {
      printError('An Uncaught Exception occurred:')(error)
      process.emit('cleanup')
    })
    process.on('unhandledRejection', error => {
      printError('An Unhandled Rejection occurred:')(error)
      process.emit('cleanup')
    })
