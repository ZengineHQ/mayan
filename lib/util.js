 ...
 * @returns {Promise<any>}
 */
module.exports.runScript = (path, script, environment) => {
  return new Promise((resolve, reject) => {
    ...
    if ('scripts' in pkg && typeof pkg.scripts === 'object') {
      if (script in pkg.scripts && pkg.scripts[script]) {
        const env = Object.assign({}, process.env, { 'MAYA_ENV': environment })

        return exec(`npm run ${script}`, { cwd: path, env }, (err, stdout) => {
          if (err) {
            return reject(err)
          }
          resolve(stdout)
 ...
 * @returns {Promise<any>}
 */
module.exports.runSequential = (arr, ...args) => {
