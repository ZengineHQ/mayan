...
exports.handleScriptOutput = res => {
  if (Array.isArray(res)) {
    res.forEach(r => console.log(kleur.bgGreen(r)))
  } else if (res.toString().length) {
    console.log(kleur.bgGreen(res.toString()))
  }
}
...
exports.runSequential = (arr, ...args) => {
  return arr.reduce((promise, func) => {
    return promise.then(() => func(...args)).catch(err => console.error(err))
  }, Promise.resolve())
}
