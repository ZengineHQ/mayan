...
exports.checkNvm = async (path) => {
  try {
    ...
    if (compare(`${version.trim()}`, this.supportedNode(), '<')) {
      console.log(kleur.yellow(`Your node version ${version} is not supported. Use the latest version >= ${this.supportedNode()} and rebuild node_modules.`))
    }
  } catch (error) {
    console.log(kleur.yellow(`No .nvmrc file found. Please add one to the root of your plugin to help development. Using Node ${this.supportedNode()} or greater.`))
  }
}
...
exports.checkNpmVersion = async () => {
  try {
    ...
    if (compare(`${version.trim().replace('v', '')}`, this.supportedNpm(), '<')) {
      console.log(kleur.yellow(`Your NPM version ${version} is not supported (> ${this.supportedNpm()}). Use the latest version npm install npm@8 and rebuild node_modules ${process.version}.`))
    }
  } catch (error) {
