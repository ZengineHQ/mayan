...
exports.checkNvm = async (path) => {
  try {
    ...
    const versionNum = version ? parseFloat(version.stdout.replace('v', '')) : 0;
...
exports.checkNpmVersion = async () => {
  try {
    ...
    if (versionNum < this.supportedNpm()) {
      console.log(kleur.yellow(`Your NPM version ${versionNum} is not supported (> ${this.supportedNpm()}). Use the latest version npm install npm@8 and rebuild node_modules ${process.version}.`))
    }
  } catch (error) {
