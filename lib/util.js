...
exports.runCommand = (command, args, options) => {
  return new Promise((resolve, reject) => {
    ...
    console.log(
      kleur.gray('[%s] Executing command: "%s %s"',
        scope,
        command,
        args.join(' ')
      ));
    ...
    subprocess.stdout.on('data', (data) => {
      console.log(kleur.gray('[%s] %s'), scope, data);
    });
    ...
    const listener = (code) => {
      ...
      if (successMessage) {
        console.log(kleur.blue('[%s] %s'), scope, successMessage);
      }
      resolve(code);
    };
 ...
 */
exports.checkNvm = async (path, scope) => {
  try {
    const nvmPath = `${path}`
    if (!fs.existsSync(nvmPath)) {
      throw new Error('no nvm ' + nvmPath)
    }
    const version = await exec('node --version')
    const versionNum = parseFloat(version.stdout.replace('v', ''));

    if (versionNum < this.supportedNode()) {
      console.log(kleur.yellow(`Your node version ${versionNum} is not supported. Use the latest version >= 14 and rebuild node_modules.`))
    }
  } catch (error) {
    console.log(error)
    console.log(kleur.yellow(`No .nvmrc file found. Please add one to the root of your plugin to help development. Using Node v14 or greater.`))
  }
}
 ...
 * @returns {Promise<void>}
 */
exports.checkNpmVersion = async (path, scope) => {
  try {
    const version = await exec('npm --version')
    const versionNum = parseFloat(version.stdout.replace('v', ''));
    if (versionNum < this.supportedNpm) {
      console.log(kleur.yellow(`Your NPM version ${versionNum} is not supported (> ${NPM_VERSION}). Use the latest version npm install npm@8 and rebuild node_modules ${process.version}.`))
    }
  } catch (error) {
    console.log(kleur.yellow(`Npm is either not installed or not a version >= ${this.supportedNpm()}`))
  }
}
/**
